...
class MongoConnect {
    ...
    public Find(find_value:object):Data{
...
async function CreateCard(titleRu:string,
    ...
    const DB = new MongoConnect('flowers', 'flowers', FlowersSchema);
    await DB.StartConnect();
    await DB.Create({titleRu: titleRu,
                                titleEn: titleEn,
...
async function Update(titleRu:string,
                      titleEn:string,
                      descriptionRu:string,
                      descriptionEn:string,
                      img:string):Promise<void>{
...
class NodeServer{
    ...
    public Api(url:string, response:Data | 'return',
               callback:Function = () => {},
               params:any[] = []):void{
...
Server.Api('/api/create_user/:full_name/:phone/:content',
    {status: true},
             CreateUser,
     ['full_name', 'phone', 'content']);

Server.Api('/api/checkadmin/:name/:password',
      'return',
               CheckAdmin,
       ['name', 'password']);

Server.Api('/api/checkadminkey/:key',
      'return',
               CheckAdminKey,
       ['key']);

Server.Api('/api/flupdate/:titleRu/:titleEn/:descriptionRu/:descriptionEn/:img',
      {status: true},
               Update,
       ['titleRu', 'titleEn', 'descriptionRu', 'descriptionEn', 'img']);

Server.Api('/api/delcard/:img',
      {status: true},
               DeleteCard,
       ['img']);

Server.Api('/api/langstatic/:lang',
      'return',
               getAllStaticText,
       ['lang']);
